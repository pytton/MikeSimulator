...
void PositionBook::fillposition(int posprice, long fillprice, long filledamount)
{
	//NOT IMPLEMENTED YET. FILLS HANDLED BY CALLING INDIVIDUAL MikePosition elements inside the orderbook vector
	COMMENTSOFF std::cout << "NOT IMPLEMENTED YET. FILLS HANDLED BY CALLING INDIVIDUAL MikePosition elements inside the orderbook vector" << std::endl;
}

long PositionBook::AllOpenPL(long bidprice, long askprice)
{
	using namespace std;
	...
	if (bidprice != prevbidprice || askprice != prevaskprice)
	{
		...
		COMMENTSOFF cout << "AllOpenPL called" << endl;
...
long PositionBook::AllClosedPL(long bidprice, long askprice)
{
	static long prevbidprice = 0;
	static long prevaskprice = 0; //for speed - to calculate only if bid or ask price has changed
...
long PositionBook::AllTotalPL(long bidprice, long askprice)
{
	static long prevbidprice = 0;
	static long prevaskprice = 0; //for speed - to calculate only if bid or ask price has changed
...
long PositionBook::TotalOpenPos()
{
